fn solution(mut nums: Vec<i32>, n: i32) -> i32 {
    let mut patches = 0;
...
fn main() {
    assert_eq!(solution(vec![1, 3], 6), 1);
    assert_eq!(solution(vec![1, 5, 10], 20), 2);
    assert_eq!(solution(vec![1, 2, 2], 5), 0);

    assert_eq!(solution(vec![1, 2, 31, 33], i32::MAX), 28);
}
